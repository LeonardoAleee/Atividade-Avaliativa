...
struct Node
{
    ...
    // Construtor do nó que inicializa o nó com um dado e configura os ponteiros e a cor
...
private:
    ...
    Node *TNULL; // Ponteiro para o nó que representa o valor nulo na árvore (sentinela preta)
    ...
    Node* searchTreeHelper(Node* node, int key)
    {
        ...
        {  // Se o nó é TNULL ou contém a chave procurada
    ...
    void balanceInsert(Node *k)
    {
        ...
        {  // Enquanto o pai do nó k for vermelho
            if (k -> parent == k -> parent -> parent -> right)
            {  // Se o pai de k é o filho direito do avô
                ...
                {  // Se o tio é preto
                    if (k == k -> parent -> left)
                    {  // Se k é o filho esquerdo do pai
            ...
            {  // Se o pai de k é o filho esquerdo do avô
...
public:
    ...
    Node* maximum(Node* node)
    {
        ...
        {  // Vai para o filho mais à direita
            node = node -> right;
        }
    ...
    Node* successor(Node* x)
    {
        ...
        {  // Move para cima até encontrar um pai que é o filho esquerdo
            x = y;
            y = y -> parent;
        }
    ...
    Node* predecessor(Node* x)
    {
        ...
        {  // Move para cima até encontrar um pai que é o filho direito
            x = y;
            y = y -> parent;
        }
    ...
    void leftRotate(Node *x)
    {
        ...
        {  // Se x é o filho esquerdo do pai
            x -> parent -> left = y;  // Atualiza o filho esquerdo do pai para y
        }
        else
        {  // Se x é o filho direito do pai
    ...
    void insert(int key)
    {
        ...
        while (x != TNULL)
        {  // Enquanto x não for TNULL
            y = x;  // Atualiza y para x
            if (node -> data < x -> data)
            {  // Se a nova chave é menor que a chave de x
        ...
        {  // Se a nova chave é menor que a chave do pai
...
private:
    ...
    void balanceDelete(Node *x)
    {
        ...
        {  // Enquanto x não for a raiz e sua cor for BLACK
            if (x == x -> parent -> left)
            {  // Se x é o filho esquerdo do pai
                ...
                else
                {
                    ...
                    {  // Se o filho direito do irmão é preto
            ...
            {  // Se x é o filho direito do pai
                ...
                else
                {
                    ...
                    {  // Se o filho esquerdo do irmão é preto
                        s -> right -> color = BLACK;  // O filho direito do irmão vira preto
                        s -> color = RED;  // O irmão vira vermelho
    ...
    void rbTransplant(Node *u, Node *v)
    {
        ...
        {  // Se u é o filho esquerdo do pai
            u -> parent -> left = v;  // v se torna o novo filho esquerdo
        }
        else
        {  // Se u é o filho direito do pai
    ...
    void deleteNodeHelper(Node *node, int key)
    {
        ...
        while (node != TNULL)
        {  // Procura o nó com a chave fornecida
            if (node -> data == key)
            {
        ...
        {  // Se o nó a ser excluído tem dois filhos
            ...
            if (y -> parent == z)
            {  // Se o sucessor é filho direito de z
                x -> parent = y;  // Atualiza o pai de x
            }
            else
            {  // Se o sucessor não é filho direito de z
        ...
        {  // Se a cor original de y era BLACK
...
int main()
{
    RBTree bst;  // Cria uma nova árvore rubro-negra
    ...
    bst.printTree();  // Imprime a árvore após as inserções
    std::cout << std::endl << "After deleting" << std::endl;
    bst.deleteNode(40);  // Exclui o nó com a chave 40 da árvore
    bst.printTree();  // Imprime a árvore após a exclusão
}
